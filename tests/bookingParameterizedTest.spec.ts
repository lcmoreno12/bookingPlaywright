...
const testCases = [
  { location: 'Mar del Plata', checkin: new Date('10 January 2025'), checkout: new Date('25 January 2025'), adultQuantity: 2, roomQuantity: 3, childrenQuantity: 1, childrenAge: ['10'] },
...
testCases.forEach(({ location, checkin, checkout, adultQuantity, roomQuantity, childrenQuantity, childrenAge }) => {
  test(`Searching in booking -Parameterized - testing with ${location}`, async ({ page }) => {
    ...
    await searchBar.setOccupancy(adultQuantity, roomQuantity, childrenQuantity, childrenAge);
